/*!
  @title com.philips.HttpLibrary.js
  @version 2.01
  @author Philips

  Timestamp: 201003090256

  %full_name: leu07hc1#1/ascii/com.philips.HttpLibrary.js/10 %
  %created_by: gli %
  %version: 10 %
  %date_created: Wed Oct 20 13:41:37 2010 %
...
((function () {
  ...
  function completeRequest(aRequest)
  {
    var i, len, req;
    len = requests.length;
    for (i = 0; i < len; i += 1) {
  ...
  function abortIdle(aTimeout)
  {
    var now, i, len, req, broken = [];
    now = (new Date()).getTime();
    len = requests.length;
    for (i = 0; i < len; i += 1) {
      req = requests[i];
      if ((now - req.time) > aTimeout) {
        broken.unshift(req);
      }
    }
    len = broken.length;
